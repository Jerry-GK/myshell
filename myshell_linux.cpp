...
#include <string.h>
#define MAXSIZE 1024
#define PARENTPATH "\\bin\\bash" //父进程
...
int cmp_time(const time_t& t1, const time_t& t2);//比较两个时间变量。-1：小于；0：等于；1：大于
...
void Init(int argc_, char* argv_[])
{
    ...
    //获取程序自身的路径(linux)
    char buffer[MAXSIZE];
    ssize_t len = readlink("/proc/self/exe", buffer, MAXSIZE);//获取自身位置
    buffer[len] = '\0';
